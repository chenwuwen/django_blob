    ...
    if ($(".commitCommentForm textarea").val().trim() != '' && $(".commitCommentForm textarea").val().trim() != undefined && $(".commitCommentForm textarea").val().trim() != null) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status) {
                    $(".commitCommentForm textarea").val("")
                    getComment()
                }
            }
        })
...
//回复框的隐藏与显示 此处有一个坑(原写法：$('.reply_button').on('click', function ()),那就是 on 方法不能绑定jquery动态新增的dom元素,
// 也就是对于下面getComment()方法改变的dom,点击回复是不起作用的，解决方法：https://blog.csdn.net/lixin2151408/article/details/71411564
$('#comment_view_area').on('click','.reply_button', function () {
...
//提交回复
$('#comment_view_area').on('click','.commitReplyForm input[type="button"]', function () {
...
function getComment() {
    $.ajax({
        ...
        success: function (ret) {
            if (ret.status) {
                comment_tree(ret.data).then(function (Dom) {  //Promise回调函数
                    console.log(Dom)
...
function comment_tree(comment_query_list) {
    var promise = new Promise(function (resolve, reject) {
        ...
        $.each(comment_query_list, function (index, comment) {
            ...
            html += '#' + (index * 1 + 1) + '楼 <a href="javascript:;">赞(18)</a> <a href="javascript:;">反对(0)</a>'
            html += '</div>'
            ...
            html += recursion(comment.children, (index * 1 + 1))
...
function recursion(comment_reply_list, root_index) {
    ...
    $.each(comment_reply_list, function (index, reply) {
        ...
        leaf_html += '#' + root_index + '楼-' + (index * 1 + 1) + '<a href="javascript:;">赞(1)</a> <a href="javascript:;">反对(18)</a>'
